...
cpd_web_server = TabularCPD(variable='Web-Server', variable_card=2,
                            values=[[1, 1-5/100, 1-1/100, 1-max(5/100,1/100)],  # Not compromised if E-store and Company Website is not compromised
                                    [0, 5/100, 1/100, max(5/100,1/100)]],  # skill/5000 chance of being compromised if E-store is compromised 1/146 + 1/730 chance if company website is compromised
...
cpd_inventory_processor = TabularCPD(variable='Inventory Processor', variable_card=2,
                                     values=[[1, 1-6/100,1-1/100,1-max(6/100,1/100)],  # Not compromised if E-store is not compromised
                                             [0, 6/100,1/100,max(6/100,1/100)]],  # 5% chance of being compromised if E-store is compromised
...
cpd_tool_server = TabularCPD(variable='Tool Server', variable_card=2,
                             values=[[1, 1-2/100],  # Not compromised if Inventory Processor not compromised
                                      [0, 2/100]], # 2% chance of being compromised if Inventory Processor is compromised
...
print(f"The probability that at least one attacker succeeds is approximately {p_at_least_one_success:.4f}")
